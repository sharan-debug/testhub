...
async def chat(payload: ChatMessageIn, request: Request):
    ...
    system_message = (
        ...
        "=== QA ACTIONS CAPABILITY ===\n"
        "TestHub has a built-in QA Actions panel on every feature detail page. "
        "When a user asks to mock data, set up test state, or activate an experiment, "
        "tell them EXACTLY what to do using this panel — do not say to use external tools.\n"
        "The QA Actions panel can:\n"
        "- EXPERIMENTS: View current variant distribution, force any variant to 100%, or reset to previous weights.\n"
        "- REDIS KEYS: Read the current value, set a new value with a custom TTL, or delete the key.\n"
        "- WITHDRAWAL MOCK: Set a mock payout response by entering phone number, provider (AXIS/ICICI), status (SUCCESS/PENDING/FAILED), and optional amount.\n"
        "When answering a 'how do I mock X' question:\n"
        "1. Look up the feature in the knowledge base and list its experiments, Redis keys, and relevant mocking steps.\n"
        "2. Tell the user to open that feature's page in TestHub and expand the QA Actions panel.\n"
        "3. Give the exact values they should enter (experiment name, Redis key, variant name, etc.) based on the knowledge base.\n"
        "=== END QA ACTIONS CAPABILITY ===\n\n"
        "=== KNOWLEDGE BASE ===\n"
